    ...
    if tool_calls:
        for tc in tool_calls:
            if isinstance(tc, dict):
                ...
                # Skip tool_uses with empty names - Kiro API rejects them.
                # This happens when clients echo back malformed tool_calls
                # (e.g., Codex receiving empty-name tool_calls from streaming).
                if not name:
                    tool_use_id = tc.get("id", "unknown")
                    logger.warning(
                        f"Skipping tool_use with empty name (toolUseId={tool_use_id}). "
                        f"Kiro API requires non-empty tool names."
                    )
                    continue
                ...
                tool_uses.append(
                    {"name": name, "input": input_data, "toolUseId": tc.get("id", "")}
                )
...
def _collect_tool_use_ids(
    content: Any, tool_calls: Optional[List[Dict[str, Any]]] = None
) -> set:
    """
    Collects all tool_use IDs from an assistant message, including empty-name ones.

    Used to detect which IDs were skipped by extract_tool_uses_from_message()
    so their corresponding tool_results can be filtered out.
...
def build_kiro_history(
    ...
    Returns:
        ...
        - Set of tool_use IDs that were skipped due to empty names
    """
    history = []
    skipped_tool_use_ids: set = set()
    ...
    for msg in messages:
        if msg.role == "user":
            ...
            if msg.tool_results:
                ...
                if kiro_tool_results and skipped_tool_use_ids:
                    filtered = [
                        tr
                        for tr in kiro_tool_results
                        if tr.get("toolUseId", "") not in skipped_tool_use_ids
                    ]
                    dropped = len(kiro_tool_results) - len(filtered)
                    if dropped > 0:
                        logger.debug(
                            f"Filtered {dropped} orphaned tool_result(s) "
                            f"referencing skipped empty-name tool_uses"
                        )
                    kiro_tool_results = filtered
            ...
            # Process tool_calls — track IDs of skipped empty-name tool_uses
            # so we can filter orphaned tool_results in subsequent user messages
            all_tool_use_ids = _collect_tool_use_ids(msg.content, msg.tool_calls)
            tool_uses = extract_tool_uses_from_message(msg.content, msg.tool_calls)
            valid_tool_use_ids = {tu["toolUseId"] for tu in tool_uses}
            skipped_tool_use_ids = all_tool_use_ids - valid_tool_use_ids
    ...
    return history, skipped_tool_use_ids
...
def build_kiro_payload(
    ...
    history, skipped_tool_use_ids = build_kiro_history(history_messages, model_id)
    ...
    if current_message.tool_results:
        ...
        if kiro_tool_results and skipped_tool_use_ids:
            ...
            if dropped > 0:
                logger.debug(
                    f"Filtered {dropped} orphaned tool_result(s) from current message "
                    f"referencing skipped empty-name tool_uses"
                )
            kiro_tool_results = filtered
        if kiro_tool_results:
