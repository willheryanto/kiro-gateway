...
async def stream_kiro_to_openai_internal(
    ...
    try:
        ...
        # Filter out tool_calls with empty names — Kiro API sometimes streams
        # tool_use events where the name gets lost during parsing. Sending these
        # to clients causes them to echo back empty-name tool_calls, which Kiro
        # rejects on the next turn with "Improperly formed request".
        if all_tool_calls:
            valid_tool_calls = [
                tc for tc in all_tool_calls if (tc.get("function") or {}).get("name")
            ]
            dropped = len(all_tool_calls) - len(valid_tool_calls)
            if dropped > 0:
                logger.warning(
                    f"Filtered {dropped} tool_call(s) with empty names from streaming response"
                )
            all_tool_calls = valid_tool_calls
