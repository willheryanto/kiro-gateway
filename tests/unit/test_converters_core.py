...
class TestExtractToolUses:
    ...
    def test_skips_empty_name_tool_calls(self):
        """
        What it does: Verifies that tool_calls with empty names are filtered out.
        Purpose: Codex echoes back empty-name tool_calls from malformed streaming
                 responses. Kiro API rejects these with "Improperly formed request".
        """
        print("Setup: tool_calls with empty and valid names...")
        tool_calls = [
            {
                "id": "call_valid",
            ...
            {"id": "call_none", "function": {"name": None, "arguments": "{}"}},
        ]
        ...
        assert result[0]["toolUseId"] == "call_valid"

    def test_skips_empty_name_content_tool_use(self):
        """
        What it does: Verifies that content tool_use blocks with empty names are filtered.
        Purpose: Same protection for Anthropic-format tool_use blocks.
        """
        print("Setup: Content with empty-name and valid tool_use...")
        content = [
            {
                "type": "tool_use",
                "id": "call_valid",
            ...
            {"type": "tool_use", "id": "call_empty", "name": "", "input": {}},
        ]
    ...
    def test_all_empty_names_returns_empty_list(self):
        """
        What it does: Verifies that all-empty-name tool_calls produce empty result.
        Purpose: When all tool_calls have empty names, no toolUses should be sent to Kiro.
        """
        print("Setup: All tool_calls with empty names...")
        tool_calls = [
...
class TestBuildKiroHistoryEmptyToolNames:
    """Tests for build_kiro_history filtering of empty-name tool_uses and orphaned tool_results."""

    def test_filters_empty_name_tool_uses_from_history(self):
        """
        What it does: Verifies that assistant messages with empty-name tool_calls
                      don't produce toolUses in Kiro history.
        Purpose: Prevents "Improperly formed request" from Kiro API.
        """
        print("Setup: Assistant with empty-name tool_calls...")
        messages = [
            UnifiedMessage(role="user", content="Hello"),
            UnifiedMessage(
                role="assistant",
        ...
        result, skipped_ids = build_kiro_history(messages, "claude-sonnet-4")

        print(f"Result: {result}")
        assert len(result) == 2
        assistant_msg = result[1]["assistantResponseMessage"]
        assert "toolUses" not in assistant_msg
        assert "call_empty" in skipped_ids

    def test_filters_orphaned_tool_results(self):
        """
        What it does: Verifies that tool_results referencing skipped empty-name
                      tool_uses are filtered from user messages.
        Purpose: Kiro API rejects tool_results without matching tool_uses.
        """
        print("Setup: Conversation with empty-name tool_call and its result...")
        messages = [
            UnifiedMessage(role="user", content="Hello"),
            UnifiedMessage(
                role="assistant",
        ...
        assistant_msg = result[1]["assistantResponseMessage"]
        assert len(assistant_msg.get("toolUses", [])) == 1
        assert assistant_msg["toolUses"][0]["name"] == "bash"

        user_msg = result[2]["userInputMessage"]
        tool_results = user_msg.get("userInputMessageContext", {}).get(
            "toolResults", []
        )
        assert len(tool_results) == 1
        assert tool_results[0]["toolUseId"] == "call_valid"

    def test_all_empty_names_removes_all_tool_results(self):
        """
        What it does: Verifies that when all tool_calls have empty names,
                      all corresponding tool_results are also removed.
        Purpose: Complete cleanup of malformed tool conversation.
        """
        print("Setup: All empty-name tool_calls...")
        messages = [
            UnifiedMessage(role="user", content="Hello"),
            UnifiedMessage(
                role="assistant",
        ...
        user_msg = result[2]["userInputMessage"]
        assert (
            "userInputMessageContext" not in user_msg
            or "toolResults" not in user_msg.get("userInputMessageContext", {})
        )

    def test_mixed_valid_and_empty_preserves_valid(self):
        """
        What it does: Verifies that valid tool_calls and their results survive
                      when mixed with empty-name ones.
        Purpose: Only malformed entries are removed, valid ones pass through.
        """
        print("Setup: Mix of valid and empty-name tool_calls...")
        messages = [
            UnifiedMessage(role="user", content="test"),
            UnifiedMessage(
                role="assistant",
                ...
                tool_calls=[
                    ...
                    {"id": "empty_1", "function": {"name": "", "arguments": "{}"}},
                    {
                        "id": "valid_2",
                        "function": {"name": "write", "arguments": '{"data": "hi"}'},
                    },
                ],
            ),
            UnifiedMessage(
                ...
                content="",
                tool_results=[
                    {"type": "tool_result", "tool_use_id": "valid_1", "content": "ok"},
                    {
                        "type": "tool_result",
                        "tool_use_id": "empty_1",
                        "content": "unsupported",
        ...
        assert {tu["name"] for tu in tool_uses} == {"read", "write"}
        ...
        assert {tr["toolUseId"] for tr in tool_results} == {"valid_1", "valid_2"}
...
class TestBuildKiroHistory:
    ...
    def test_builds_user_message(self):
        ...
        print("Action: Building history...")
        result, _ = build_kiro_history(messages, "claude-sonnet-4")
    ...
    def test_handles_empty_list(self):
        ...
        print("Action: Building history...")
        result, _ = build_kiro_history([], "claude-sonnet-4")
