...
class TestStreamingOpenaiNoneProtection:
    ...
    async def test_handles_none_function_name(
        ...
        Goal: Verify tool_calls with None name are filtered out to prevent
              Kiro API "Improperly formed request" on subsequent turns.
        """
        ...
        tool_chunks = [c for c in chunks if '"tool_calls"' in c]
        assert len(tool_chunks) == 0, "None-name tool_calls should be filtered out"

        done_chunks = [c for c in chunks if '"finish_reason"' in c and '"stop"' in c]
        assert len(done_chunks) >= 1

        print("✓ None function name filtered out")
